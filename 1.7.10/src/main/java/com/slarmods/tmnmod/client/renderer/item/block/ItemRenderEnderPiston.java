...
public class ItemRenderEnderPiston implements IItemRenderer {
	...
	public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
		...
		case EQUIPPED:
		case EQUIPPED_FIRST_PERSON: {
			break; // caller expects us to render over [0,0,0] to [1,1,1], no
					// translation necessary
		}
		case ENTITY:
		case INVENTORY: {
			// translate our coordinates so that [0,0,0] to [1,1,1] translates
			// to the [-0.5, -0.5, -0.5] to [0.5, 0.5, 0.5] expected by the
			// caller.
			
			GL11.glTranslatef(-0.5F, -0.5F, -0.5F);
			mustundotranslate = true; // must undo the translation when we're
										// finished rendering
			break;
		}
		default:
		...
		if (testflagColour)
			tessellator.setColorOpaque(0, 0, 255);
		...
		// ypos face red
		IIcon iicon = TooMuchNature.ender_piston_normal.getPistonBaseIcon(TooMuchNature.modid + ":" + "ender_piston_top_normal");
