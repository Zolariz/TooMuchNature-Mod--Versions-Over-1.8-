...
public class ItemTMNSpawnEgg extends Item {
	...
	public int getColorFromItemStack(ItemStack stack, int renderPass) {
		List<Integer> colors = TMNEntityList.entityEggs.get(TMNEntityList.getClassFromID(stack.getItemDamage()));
	...
	public Entity spawnCreature(World world, int entityID, double x, double y, double z) {
		Entity entity = null;
		Class<? extends Entity> oclass = TMNEntityList.getClassFromID(entityID);
		if (TMNEntityList.entityEggs.containsKey(oclass)) {
			entity = TMNEntityList.createEntity(oclass, world);
			if (entity instanceof EntityLiving) {
				EntityLiving entityliving = (EntityLiving) entity;
				entity.setLocationAndAngles(x, y, z, MathHelper.wrapAngleTo180_float(world.rand.nextFloat() * 360.0F),
						0.0F);
	...
	/**
	 * Attempts to spawn a child when the player interacts with an entity using
	 * a custom spawn egg
	 * 
	 * @param stack
	 *            a stack containing an ItemTMNSpawnEgg item
	 * @param player
	 *            the player interacting with the entity
	 * @param entity
	 *            the entity that will spawn the child
	 * @return true if a child was spawned and the EntityInteractEvent should be
	 *         canceled
	 */
	private boolean spawnChild(World world, ItemStack itemstack, EntityPlayer player, EntityAgeable entityAgeable) {
		Class oclass = TMNEntityList.getClassFromID(itemstack.getItemDamage());
		if (oclass != null && oclass == entityAgeable.getClass()) {
			EntityAgeable child = entityAgeable.createChild(entityAgeable);
			if (child != null) {
				child.setGrowingAge(-24000);
				child.setLocationAndAngles(entityAgeable.posX, entityAgeable.posY, entityAgeable.posZ, 0.0F, 0.0F);
				if (!world.isRemote) {
	...
	public void getSubItems(Item item, CreativeTabs creativetab, List list) {
		Iterator<Class<? extends Entity>> iterator = TMNEntityList.entityEggs.keySet().iterator();
		while (iterator.hasNext()) {
			Class<? extends Entity> oclass = iterator.next();
			List<Integer> colors = TMNEntityList.entityEggs.get(oclass);
			if (colors != null && colors.size() == 2) {
				list.add(new ItemStack(item, 1, TMNEntityList.getEntityId(oclass)));
			}
		}
	}
