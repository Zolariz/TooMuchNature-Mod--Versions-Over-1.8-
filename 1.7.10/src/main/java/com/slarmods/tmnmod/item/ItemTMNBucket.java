			...
			if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
				...
				if (flag) {
					...
					if (material == Material.water && l == 0) {
						...
						return this.func_150910_a(itemstack, player, TooMuchNature.ender_water_bucket);
					}

				} else {
					if (this.isFilled == Blocks.air) {
						return new ItemStack(TooMuchNature.enderald_bucket);
					}
	...
	public boolean tryPlaceContainedLiquid(World world, int x, int y, int z) {
		if (this.isFilled == Blocks.air) {
			...
			if (!world.isAirBlock(x, y, z) && !flag) {
				...
				if (world.provider.isHellWorld && this.isFilled == TooMuchNature.ender_water_dynamic) {
					world.playSoundEffect((double) ((float) x + 0.5F),
							(double) ((float) y + 0.5F), (double) ((float) z + 0.5F), "random.fizz",
							0.5F, 2.6F + (world.rand.nextFloat() - world.rand.nextFloat()) * 0.8F);

					for (int l = 0; l < 8; ++l) {
						world.spawnParticle("largesmoke", (double) x + Math.random(),
								(double) y + Math.random(), (double) z + Math.random(), 0.0D, 0.0D,
								0.0D);
					}
				} else {
					...
					world.setBlock(x, y, z, this.isFilled, 0, 3);
				}
