...
import com.slarmods.tmnmod.util.GameLogHelper;
...
public class ChunkProviderEnderlands implements IChunkProvider {
	...
	public ChunkProviderEnderlands(World world, long seed, boolean mapFeaturesEnabled) {
		...
		NoiseGenerator[] noiseGens = { noiseGen1, noiseGen2, noiseGen3, noisePerl, noiseGen5, noiseGen6,
				mobSpawnerNoise };
	...
	public void func_147424_a(int par1, int par2, Block[] blocks) {
		...
		// DONT EDIT THS METHOD UNLES YOU KNOW WHAT UR DOING OR MAKE A COPY
		// INCASE U MESS IT UP....
		// YOU HAVE BE WARNED !!!!!
					...
					for (int l2 = 0; l2 < 8; ++l2) {
						...
						for (int i3 = 0; i3 < 4; ++i3) {
							...
							for (int k3 = 0; k3 < 4; ++k3) {
								if ((d15 += d16) > 0.0D) {
									blocks[j3 += short1] = TooMuchNature.end_stone_smooth;
								} else if (k2 * 8 + l2 < b0) {
	...
	public void replaceBlocksForBiome(int par1, int par2, Block[] blocks, byte[] par3ArrayOfByte,
		...
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(this, par1, par2,
				blocks, par3ArrayOfByte, par4ArrayOfBiomeGenBase);
		MinecraftForge.EVENT_BUS.post(event);
		if (event.getResult() == Result.DENY)
			return;
		double d0 = 0.03125D;
		this.stoneNoise = this.noisePerl.func_151599_a(this.stoneNoise, (double) (par1 * 16), (double) (par2 * 16), 16,
				16, d0 * 2.0D, d0 * 2.0D, 1.0D);
		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				EnderBiomes biomegenbase = (EnderBiomes) par4ArrayOfBiomeGenBase[l + k * 16];
				biomegenbase.genTerrainBlocks(this.worldObj, this.rand, blocks, par3ArrayOfByte, par1 * 16 + k,
						par2 * 16 + l, this.stoneNoise[l + k * 16]);
			}
		}
	}
	...
	private void func_147423_a(int x, int y, int z) {
		...
		this.noise3 = this.noiseGen3.generateNoiseOctaves(this.noise3, x, y, z, 5, 33, 5, 8.555150000000001D,
