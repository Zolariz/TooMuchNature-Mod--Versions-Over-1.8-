...
public class RenderBlockEnderBeacon implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
		...
		for (k = 0; k < 3; ++k) {
			if (k == 0) {
				...
				renderer.setOverrideBlockTexture(renderer.getBlockIcon(TMNBlocks.end_obsidian));
			} else if (k == 1) {
				renderer.setRenderBounds(0.1875D, 0.1875D, 0.1875D, 0.8125D, 0.875D, 0.8125D);
				renderer.setOverrideBlockTexture(renderer.getBlockIcon(TMNBlocks.lower_end_portal));
			} else if (k == 2) {
