package com.theslarfab.tmnmod.entity.monster;
	...
	public void onUpdate() {
		...
		if (this.onGround && !flag) {
			...
			for (int j = 0; j < i * 8; ++j) {
				...
				this.worldObj.spawnParticle(this.getSlimeParticle(), this.posX + (double) f2, this.boundingBox.minY,
	...
	public void onCollideWithPlayer(EntityPlayer p_70100_1_) {
		if (this.canDamagePlayer()) {
			...
			if (this.canEntityBeSeen(p_70100_1_)
					&& this.getDistanceSqToEntity(p_70100_1_) < 0.6D * (double) i * 0.6D * (double) i && p_70100_1_
	...
	public boolean getCanSpawnHere() {
		...
		if (this.worldObj.getWorldInfo().getTerrainType().handleSlimeSpawnReduction(rand, worldObj)) {
			...
			if (this.getSlimeSize() == 1 || this.worldObj.difficultySetting != EnumDifficulty.PEACEFUL) {
				BiomeGenBase biomegenbase = this.worldObj.getBiomeGenForCoords(MathHelper.floor_double(this.posX),
						MathHelper.floor_double(this.posZ));
				...
				if (biomegenbase == BiomeGenBase.swampland && this.posY > 50.0D && this.posY < 70.0D
						...
						&& this.worldObj.getBlockLightValue(MathHelper.floor_double(this.posX),
								MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)) <= this.rand
										.nextInt(8)) {
